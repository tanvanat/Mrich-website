...
import { questions, MAX_SCORES, TOTAL_MAX } from "@/lib/questions-course3";
...
function detectCourse(r: Row) { return r.course ?? r.answersJson?.course ?? null; }
function detectFormId(r: Row) { return r.formId ?? r.answersJson?._meta?.formId ?? null; }
function getDisplayName(r: Row) { return r.user?.name ?? r.name ?? "—"; }
...
export default function AdminExamCourse3Client() {
  ...
  async function saveScores() {
    ...
    try {
      ...
      if (!res.ok) { alert(`บันทึกไม่สำเร็จ: ${out?.error ?? "unknown"}`); return; }
  ...
  async function unlockByNickname(nickname: string) {
    ...
    try {
      ...
      const out = await res.json().catch(() => ({}));
      if (!res.ok) { alert(`ปลดล็อกไม่สำเร็จ: ${out?.error ?? "unknown"}`); return; }
      await load();
      alert(`ปลดล็อกแล้ว: ${nick}`);
    } finally {
      setUnlocking(null);
    }
  }
  ...
  const rows = useMemo(() => {
    ...
    if (q) result = result.filter((r) => getDisplayName(r).toLowerCase().includes(q));
    return result;
  }, [data, query]);
      ...
      <div className="relative max-w-7xl mx-auto px-6 py-10">
        ...
        <div className="rounded-2xl bg-white/5 backdrop-blur-xl border border-blue-500/20 p-6 shadow-2xl mb-8">
          <div className="flex justify-between items-center flex-wrap gap-4">
            <h1 className="text-3xl font-bold">Admin — Exam Scoring (Course 3)</h1>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                ...
                {rows.length === 0 && (
                  <tr>
                    <td colSpan={6} className="p-10 text-center text-blue-300">ไม่พบข้อมูล</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        >
          ...
          >
            <div className="p-6 border-b border-blue-500/20 flex justify-between items-center">
              ...
              <button className="text-4xl text-blue-300 hover:text-white" onClick={() => setOpenRow(null)}>×</button>
            </div>
